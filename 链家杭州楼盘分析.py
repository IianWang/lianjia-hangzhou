...
# ## 出于前面得出的热销地区前五名，我主要关注余杭、萧山、江干、拱墅、西湖。我发现这五个区他们的均值呈阶梯形，其中西湖区与江干区，萧山区与余杭区的均价较为接近。所以大致上三个价位分别是46000，41000，26000。
...
# ### 通过所得的回归模型，决定系数R-squared为0.664，相关性较强，可以说通过面积均价有66.4%可信度来说明房屋的售卖程度

# ## 猜想2的结论：
# ### 就该网站的楼盘信息，绝大多数已购房的用户衡量了房屋的价格与地理位置选择了就自身而言性价比较高的地段。

# ## 猜想2的结论主要依据：
# ### 1.就之前的饼图得出，已售罄的楼盘中余杭占29.6%，萧山占21.8%，江干占12.1%，西湖和拱墅分别占7.9%。数据表明选择余杭和萧山地区的用户要明显多于江干、西湖、拱墅的总和。且均价方面，余杭和萧山平均价位在26000，江干和西湖平均价位在41000，拱墅平均价位在46000。
# ### 2.刚刚得出的平均价位与售卖程度之间的决定系数约为0.66，表明两者相关性较强。在影响购房者的所有因素中，平均价位占到66%。
...
# #### 三：通过热销TOP10的地区楼盘均价直方图，所有地区楼盘价格的箱线图以及相关性分析，寻找价格是否是导致用户所选择区域的主要原因，如果是，那么价格能在多大的程度上说明，所得结果说明面积均价是用户选择房屋位置的主要原因，其占所有原因的66%。之前的箱线图连带着展示了各个地区房屋价格分布的离散程度。
